...
- **Smart filters**: Saved filters, filter editing, same-folder and same-size group rules, similarity same-resolution and average-duration rules, and modified-time rules that persist across sessions.
- **Advanced bulk delete**: Preview-backed commands can keep the oldest, newest, shortest-duration, or longest-duration file, with modified-time fallback for equal video durations.
