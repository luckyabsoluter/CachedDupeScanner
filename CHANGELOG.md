...
- Similarity signature explanations now use shared core parsing before the UI formats readable cluster summaries.
...
- Similarity result details keep deleted members in place with only the deleted-background highlight.
- File delete actions keep generated similarity clusters in place until an explicit scan, update, or rebuild refreshes them.
